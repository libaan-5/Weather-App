...
class MyGUI:
    ...
    def get_weather(self, city, country_code=None, units=None): 
        ...
        # Error messages below    
            ...
            output_error = "Error 400: Bad request (maybe missing parameters or invalid query)."
            return output_error
        elif response.status_code == 401:
            output_error = "Error 401: Invalid API key."
            return output_error
        elif response.status_code == 404:
            output_error = "Error 404: City not found (incorrect input)."
            return output_error
        elif response.status_code == 429:
            output_error = "Error 429: Too many requests – you’ve hit the API limit."
            return output_error
        else:
            output_error = f"Error {response.status_code}: Unexpected API error."
            return output_error
